...
`state`는 대부분의 경우 상점의 중심 부분입니다. 그리고 개발자들은 대부분 앱을 나타내는 `state`를 정의하는 것으로 시작합니다. Pinia에서 `state`는 초기 값을 리턴하는 함수로 정의됩니다. 그리고 이러한 `state`는 서버, 클라이언트 모두에서 작동할 수 있습니다.
